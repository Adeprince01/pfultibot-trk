...
class TestReconnectionLogic:
    ...
    async def test_auto_reconnect_on_connection_loss(self, listener):
        ...
        mock_client.is_user_authorized.side_effect = [
            False,
            True,
        ]  # Lost, then recovered
...
class TestAutomaticRetryMechanisms:
    ...
    async def test_retry_failed_message_processing(self, listener):
        ...
        with patch("src.listener.parse_crypto_call") as mock_parse:
            ...
            result = await message_handler.handle_message_with_retry(
...
class TestReliabilityIntegration:
    ...
    async def test_end_to_end_reliability(self, listener_with_reliability):
        ...
        with (
            patch.object(listener, "auto_reconnect") as mock_reconnect,
            patch("asyncio.sleep"),
        ):
    ...
    @pytest.mark.asyncio
    async def test_graceful_shutdown_with_pending_operations(
        self, listener_with_reliability
    ):
