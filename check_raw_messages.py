...
def check_raw_messages():
    ...
    try:
        ...
        cursor = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='raw_messages'")
        ...
        for i, row in enumerate(cursor.fetchall(), 1):
            msg_text = row['message_text'][:100] + "..." if len(row['message_text']) > 100 else row['message_text']
            reply_info = f" (Reply to: {row['reply_to_message_id']})" if row['reply_to_message_id'] else ""
