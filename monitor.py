...
class EnhancedProductionStorage:
    ...
    def __init__(self, db_path: Path) -> None:
        ...
        logger.info(
            f"Enhanced storage initialized with backends: {', '.join(self.storage.active_backends)}"
        )
    ...
    def append_row(self, data: Dict[str, any]) -> None:
        ...
        try:
            ...
            active_backends = [
                name.title() for name, active in backend_status.items() if active
            ]

            logger.info(
                f"📈 CALL #{self.call_count}: {token} - {gain}x gain "
    ...
    def store_raw_message(self, message_data: Dict[str, any]) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Raw message {message_data.get('message_id')} stored successfully"
            )
        except Exception as e:
            logger.error(
                f"Failed to store raw message {message_data.get('message_id')}: {e}"
            )

    def get_raw_messages(
        self, limit: int = None, channel_id: int = None, unclassified_only: bool = False
    ) -> List[Dict[str, any]]:
    ...
    def close(self) -> None:
        ...
        logger.info(
            f"Closing enhanced storage. Total calls: {stats['total_calls']}, "
...
class EnhancedCryptoMonitor:
    ...
    async def start_monitoring(self) -> None:
        ...
        try:
            ...
            logger.info(
                "🎧 Enhanced monitoring started - listening for crypto calls..."
            )
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            print(
                f"⏱️ Uptime: {str(uptime).split('.')[0] if self.start_time else 'Unknown'}"
            )
